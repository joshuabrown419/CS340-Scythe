...
function handleGameFactionRequest(req, res) {
    if(req.query.operation === 'select'){
        ...
        db.pool.query(`UPDATE GameFaction SET
            playerID = (SELECT playerID FROM Player WHERE playerName = "` + req.query.playerName + `"), gameID = ` + req.query.gameID + `, endingCoins = ` + req.query.endingCoins + `, endingPopularity = ` + req.query.endingPopularity + `, starsPlaced = ` + req.query.starsPlaced + `, tilesControlled = ` + req.query.tilesControlled + `, faction = "` + req.query.faction + `", playerBoard = "` + req.query.playerBoard + `", resources = ` + req.query.resources + `;`, function(err, result) {
            if (err) {
                console.log(err)
                res.sendStatus(400);
                return;
